...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl glass-panel rounded-3xl p-5 sm:p-6 border border-slate-700/80 shadow-2xl flex flex-col my-auto space-y-4 max-h-[90vh] overflow-y-auto">
        {/* Header Close Button */}
        {mode === 'choose' ? (
          <button
            onClick={() => {
        ...
        <div className="text-center space-y-2 pt-2 sm:pt-0">
          ...
          {(!settings.isRoomSet || mode === 'join') && (
            <>
              <h2 className="text-lg sm:text-xl font-extrabold text-slate-100 tracking-tight">
                {mode === 'join' ? 'Gabung Workspace / Room' : 'Selamat Datang di Hopp'}
              </h2>
        ...
        {/* Current Room Badge + Copy Button + Device Button + Keluar Room Button — only show in choose mode */}
        {settings.isRoomSet && mode === 'choose' && (
          <div className="p-3.5 sm:p-4 bg-indigo-950/50 border border-indigo-500/30 rounded-2xl space-y-3 shadow-inner">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center space-x-1.5 sm:space-x-2 shrink-0 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={handleCopyCurrentRoom}
                  className="flex-1 sm:flex-none flex items-center justify-center space-x-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl text-xs font-semibold transition-all shrink-0"
                >
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center space-x-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/30 shrink-0"
                >
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center space-x-1 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 hover:text-white border border-rose-500/30 hover:border-rose-500/60 rounded-xl text-xs font-semibold transition-all shrink-0"
                >
            ...
            {/* Leave Room Confirmation Panel */}
            {confirmLeave && (
              <div className="p-3 rounded-xl bg-red-950/60 border border-red-500/50 space-y-2 animate-fadeIn">
                ...
                <div className="flex items-center space-x-2 pt-1">
                  <button
                    ...
                    className="w-1/2 py-1.5 text-xs font-semibold text-slate-400 hover:text-white bg-slate-900 hover:bg-slate-800 rounded-lg border border-slate-800 transition-all"
                  >
                    Batal
                  </button>
                  <button
                    ...
                    className="w-1/2 py-1.5 text-xs font-bold text-white bg-red-600 hover:bg-red-500 rounded-lg border border-red-500 transition-all flex items-center justify-center space-x-1.5"
                  >
        ...
        {/* Visual Divider between Current Room Card and Other Room Actions */}
        {settings.isRoomSet && mode === 'choose' && (
          <div className="relative flex items-center justify-center py-1">
            <div className="border-t border-slate-800/80 w-full" />
            <span className="bg-slate-950 px-3 py-0.5 text-[10px] uppercase tracking-wider text-slate-500 font-extrabold shrink-0 rounded-full border border-slate-800 shadow-sm">
